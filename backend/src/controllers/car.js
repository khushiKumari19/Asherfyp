...
// const ABI = require('../../../admin/src/Containers/Metamask/solidity/artifacts/src/solidity/contracts/ERC20Token.sol/ERC20Token.json')
...
exports.createCar = async (req, res) => {
  try {
    ...
    // console.log(totalSupply, contractHash);
    const car = new Car({
      name, model, company, country, color, engine, feulType, tokenPrice, totalPrice,
      carPictures, noOfSeats, noOfDoors, description, totalSupply: parseInt(totalSupply), 
      contractHash, createdBy
    });
    console.log(car);
    car.save((error, car) => {
      if (error) return res.status(400).json({ error });
